...
public abstract class BaseCallBackService extends HttpServlet {

	private static final long serialVersionUID = 1L;

	public static Logger log = Logger.getLogger(BaseCallBackService.class);
	...
	protected void sendResponse(JSONObject sResponse, HttpServletRequest request, HttpServletResponse res) {

		try {

			res.setContentType("application/json; charset=UTF-8");

			res.getWriter().print(sResponse.toString());
			log.info(request.getServletPath() + "-- thread id -- " + Thread.currentThread().getId()
					+ " -- response sent -- " + sResponse.toString());
